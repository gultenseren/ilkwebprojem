...
<?php
	...
	function fillTable($coursename){
		...
		$mysqli = new mysqli("sql313.epizy.com","epiz_22222572","kaancetin123","epiz_22222572_stdmgdb");
			...
			$courseid=mysqli_query($mysqli,"SELECT id FROM course WHERE label='$selectCourse'");
	...
	if($mysqli->connect_errno)
		die("Connection failed".$mysqli->connect_error);
			...
			if(isset($_POST['studentid'])){
				...
				if($_POST['studentid']=="" || $_POST['studentid']==null){
					...
					echo "<script> alert('choose student to update');</script>";
							...
							$query = "UPDATE `enroll` SET `score`=$score WHERE `student_id`=$studentid AND `course_id`='$cid'";
			...
			$query = "SELECT id,first_name,last_name,score FROM student AS s INNER JOIN enroll AS c ON s.id=c.student_id WHERE course_id=$cid";
