...
<ul>
	...
	<div id="studenttable">
           ...
           <?php 
					...
					if(isset($_POST["Remove"])==1 && $_POST["Remove"]=="Remove"){ 
						...
						insertt($first_name,$last_name,$sex,$birthdate,$phone,$address);
	...
	$query = "SELECT * FROM student;";
	//$query .= "SELECT name as FistName,surname as SecondName FROM contact;";
	//$query .= "SELECT name,surname,city FROM contact where language='Hindi';";
...
function deletee($id){
	...
	function insertt($first_name,$last_name,$sex,$birthdate,$phone,$address){
		
		
		if($first_name==null || $first_name=='' || $last_name==''|| $last_name==''||
